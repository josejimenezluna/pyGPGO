...
\section{Discussion}
In these experiments, we have shown that Bayesian Optimization is more efficient when optimizing the parameters of a machine-learning model when taking into account the number of evaluations. This fact can be seen by the fact that we used the same evaluation over datasets of diverse enough nature. However, random search is embarrassingly parallel over threads, and therefore can potentially outperform these techniques when given extra computational power. My personal feeling is that further work needs to be done in parallelizing Bayesian Optimization strategies. This means sharing evaluation information over threads, so that in each iteration, every evaluation fits a similar surrogate model.
...
pyGPGO, while a very complete software for Bayesian Optimization remains still software in development. In particular, there are several improvements we can tackle on:
...
\begin{enumerate}
	...
	\item Support for more diverse acquisition functions. Entropy-based acquisition functions are commonly used.
\end{enumerate}
